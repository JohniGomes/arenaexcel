...
export class EmailService {
  ...
  constructor() {
    ...
    if (!resendApiKey) {
      this.logger.warn(
        '⚠️  Resend API Key não configurada. E-mails serão apenas logados (não enviados).',
      );
      this.logger.warn(
        'Configure RESEND_API_KEY no .env para enviar e-mails reais.',
      );
      return;
    }
  ...
  async sendPasswordResetEmail(
    ...
    // Link aponta para o backend que retorna HTML com deep link
    ...
    if (!this.resend) {
      this.logger.warn(
        `📧 [SIMULADO] E-mail de reset para: ${email}\nToken: ${resetToken}\nURL: ${resetUrl}`,
      );
      return;
    }
    ...
    try {
      ...
      if (error) {
        this.logger.error(
          `❌ Erro Resend ao enviar para ${email}: ${error?.message ?? JSON.stringify(error)}`,
        );
      ...
      this.logger.error(
        `❌ Erro ao enviar e-mail para ${email}: ${error?.message ?? 'Erro desconhecido'}`,
        error?.stack,
      );
      throw new Error('Falha ao enviar e-mail. Tente novamente mais tarde.');
    }
