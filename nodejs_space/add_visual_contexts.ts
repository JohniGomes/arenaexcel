...
// Contextos visuais HTML para questões MULTIPLE_CHOICE e DRAG_AND_DROP
const visualContexts: Record<string, Record<number, string>> = {
  'excel-do-zero': {
    1: `
  ...
  for (const [slug, contexts] of Object.entries(visualContexts)) {
    ...
    for (const [orderStr, htmlContext] of Object.entries(contexts)) {
      ...
      try {
        ...
        // Atualizar ou adicionar spreadsheetContext
        let newContext = htmlContext;
        
        // Se já tem context em JSON, adicionar o htmlView
